# LUXA Sales — Frontend ↔ Backend Integration Plan (Master Index)
...
1. **Backend features not in the frontend → prepare the frontend** — Phases 2–4 (API layer, adapters, context rewrites).
2. **Frontend features not in the backend → create them in the backend** — Phase 1 (backend gaps, TDD).
3. **Connect and test everything** — Phase 5 (full test/build gates + smoke tests).
...
|---|---|---|---|---|
| 0 | `PLAN_00_ENV_AND_CONTRACT.md` | — | Env/CORS/rewrite, frontend test harness, API contract types | — |
...
| 3 | `PLAN_03_ADAPTERS_AND_TYPES_TDD.md` | 1 | Adapter layer backend shapes → UI shapes; type sync; write-guards | 1 |
| 4 | `PLAN_04_CONTEXT_REWRITES_TDD.md` | 1 | Contexts/pages → React Query; routing fixes; cleanup | 2, 3 |
...
3. **Investors stay fully mock** — no backend investor module exists. Block investor login with a friendly "coming soon" message.
...
5. **TDD is mandatory.** RED → GREEN → VERIFY for every task in Phases 1–4. Frontend gets a Vitest harness in Phase 0 (no test framework exists today).
...
| `TeamMember` has **no** `email` column — only `userId` FK; `list()` does not join `User` | `team-member.entity.ts`, `team.repository.ts:73-104` |
...
| Frontend `Permission` union = `view-products`, `view-out-of-stock`, `edit-products`, `view-sales-history`, `checkout-sales`, `view-inventory`, `assign-roles` — out of sync | `teamTypes.ts:8-15` |
| `UpdatePermissionsDtoSchema` / `InviteMemberDtoSchema` validate with `z.enum(TEAM_PERMISSIONS)` — sending `checkout-sales`/`view-out-of-stock` returns 400 | `update-permissions.dto.ts`, `invite-member.dto.ts` |
...
| Frontend `LoginCredentials`/`SignupData` send `role` → strict Zod 400 | `auth.ts:21,29`, `LoginForm.tsx:35`, `SignupForm.tsx:43-50` |
...
| `GET /auth/me` returns only the JWT payload — no `firstName/lastName/avatar` | `auth.controller.ts:100-104` |
| Middleware reads `luxa_auth` cookie; **nothing ever sets it**; all routes fall through | `middleware.ts:13,22` |
| `DataContext.tsx` exists but is unmounted — dead file | `contexts/DataContext.tsx`, `providers.tsx` |
| Frontend `UserProfile.notificationPreferences` has `sms`; backend does not | `profileTypes.ts:19`, `user-profile.entity.ts:43-63` |
| `UpdatePreferencesDtoSchema` notificationPreferences is `.strict()` with no `sms` → sending `sms` = 400 | `update-preferences.dto.ts:3-13` |
| `UpdateProfileDtoSchema` has no `name` — must send `firstName`/`lastName` | `update-profile.dto.ts` |
...
| `GET /sales` returns `{ data, pagination, summary }` — sales adds a `summary` field | `sales.repository.ts:146-155` |
...
| Post-login redirect sends everyone non-investor to `/dashboard` → apprentices will 403 | `AuthContext.tsx:60` |
...
## TDD conventions (apply to every task in Phases 1–4)

Each task follows a strict RED → GREEN → VERIFY loop:

1. **RED** — Write the failing test first.
   - Backend: colocated `*.spec.ts` (jest, already configured). Run only that file: `npx jest <path> --runInBand`.
   - Frontend: colocated `*.test.ts(x)` (Vitest — harness added in Phase 0). Run only that file: `npx vitest run <path>`.
   - Assert the test fails for the right reason (feature missing, not a broken harness).
2. **GREEN** — Write the minimal implementation to make that test pass. Run the same targeted test.
3. **VERIFY** — Run the full gate for the affected repo:
   - Backend: `npm test` then `npm run check` (includes `check:tdd` which fails if any source file lacks a spec — so never delete spec files).
...
| Money precision | Postgres `decimal` → string in JSON. Adapters call `Number()` then `Math.round(n * 100) / 100`. |
| Zod strict DTOs | Every unknown field = 400. Write-guards in Phase 3: strip `role`, `sms`, deprecated permission aliases, `name`→`firstName/lastName`. |
...
## Deferred (not blocking — Phase 6 in old numbering)

Backend modules: **Investors** (auth, ROI), **Withdrawals** (approval workflow), **Reports** (PDF/CSV), **Data management** (backup/import/export), **AI insights**, **Activity logs**, **Dashboard customisation** persistence, notification producers for `alert`/`ai`/`system`, real **invite-acceptance** flow (invited staff cannot log in — guard the UI with a "pending activation" state), server-side **pagination** for inventory/sales contexts, `sms` preference persistence (frontend strips it), `lastActive`/`invitedBy` columns on `team_members`.

---

## Repository hygiene

- Both `sales-backend/` and `sales-frontend/` are independent git repos. Commit after each phase's VERIFY step with a descriptive message. Never commit secrets (`.env` is gitignored; use `.env.local`/`.env.example` patterns).