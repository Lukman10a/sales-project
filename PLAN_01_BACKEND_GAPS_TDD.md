# Phase 1 â€” Backend Gaps (Frontend Features â†’ Backend) â€” TDD

**File:** `PLAN_01_BACKEND_GAPS_TDD.md`
**Bucket 2.** Every frontend feature that has no backend implementation gets one now, so Phases 2â€“4 build against a complete contract.
**All work in:** `apps/backend/`.

Per task: **RED** (write failing `*.spec.ts`, run it, confirm it fails) â†’ **GREEN** (implement) â†’ **VERIFY** (`npm test`, then `npm run check`). Never delete a spec file â€” `npm run check:tdd` enforces their existence.

---

## 1.1 Inventory: persist `image`, `lastRestocked`, `confirmedByApprentice`

**Why (B2):** The entity has the columns but the DTOs reject them AND `InventoryService.create()/update()` never copy them â€” adding DTO fields alone would silently drop data. Both sides must change.
...
- Run `npx jest inventory.service.spec --runInBand` â†’ fails (fields dropped).

**GREEN**
- `src/inventory/dto/create-inventory.dto.ts` â€” add:
  ```ts
  image: z.string().optional(),
  lastRestocked: z.coerce.date().optional(),
  confirmedByApprentice: z.boolean().optional(),
  ```
- `src/inventory/dto/update-inventory.dto.ts` â€” add the same three fields (each `.optional()`).
...
**VERIFY** â€” `npm test` green; `npm run check` green.

---

## 1.2 Auth: `staffRole` in JWT + `CurrentUserPayload`

**Why (B1):** The frontend permission system reads `user.staffRole`. Today it lives only on the `User` entity and is never emitted â€” after the auth rewrite every apprentice would appear staffRole-less and lose all permissions.

**RED**
- `src/auth/jwt.strategy.spec.ts`: add test â€” `validate()` maps a payload containing `staffRole` into `CurrentUserPayload.staffRole`.
- `src/auth/auth.service.spec.ts`: add test â€” `generateTokens()` (via `register`/`login`) includes `staffRole` in the access token payload for a staff user.
- Run both â†’ fail.
...
**Why (B3):** `GET /auth/me` currently returns only the JWT payload (no `firstName/lastName/avatar`). `AuthContext` will hydrate the session from `/auth/me` in Phase 2 â€” it must return the complete user shape.

**RED**
- `src/auth/auth.controller.spec.ts`: add test â€” `GET /auth/me` returns `firstName`, `lastName`, `avatar` (not just the payload).
- Run â†’ fail.
...
**RED**
- `src/sales/sales.repository.spec.ts` (or `sales.service.spec.ts`): add test â€” after `create()` a sale with a known product, the returned `items` include `productName` and a computed `total` (`price * quantity`).
- Run â†’ fail.
...
- `src/entities/sale-item.entity.ts`: add
  ...
  total!: number;
  ```
  (Requires a small migration â€” see note below.)
...
> **Migration note:** this repo has no migration runner configured â€” the schema is synced (TypeORM `synchronize`/scripts). Confirm how schema changes are applied (see `database.config.ts`) and add the columns to the DB the same way the other columns were added (documented in `PHASE1_COMPLETE.md`). Same applies to 1.5.
...
**RED**
- `src/sales/sales.service.spec.ts`: add test â€” `create()` with `splitPayments`, `loyaltyPointsUsed`, `accountCredit` returns a sale persisting them (no 400 from validation).
...
**RED**
- `src/sales/sales.repository.spec.ts`: add test â€” `list()` returns each sale with an `itemCount` matching its `sale_items` rows.
- Run â†’ fail.
...
**Why (Gap #1):** `TeamMember` has no `email`; the team UI requires it. Join `User` on read â€” no migration.

**RED**
- `src/team/team.repository.spec.ts`: add test â€” `list()` returns members whose response includes `email` from the joined `User`.
- Run â†’ fail.
...
1. `npm run check` (backend) is fully green after every task â€” each new source file has a spec.
2. Manual API smoke (curl against `npm run start:dev`):
   - Register owner â†’ create inventory item **with `image`** â†’ item returns `image`.
   - `GET /auth/me` returns `firstName`/`lastName`/`avatar`.
   - Access-token JWT payload contains `staffRole` when applicable.
   - Record a sale â†’ `GET /sales/:id` items have `productName` + `total`; `GET /sales` rows have `itemCount`.
